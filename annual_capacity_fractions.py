# -*- coding: utf-8 -*-
"""
Created on Sun Mar 27 16:22:19 2016
...
graphing_data2 = pd.DataFrame(columns=graph_data[0])      
      
for i in range(1,len(graph_data)):
    graphing_data2.loc[i] = graph_data[i]
    
graphing_data2.to_csv('graphing_data2.csv')
print 'done'
## prime mover graph
...
#
## PM graph 
#PMs = list(set(graphing_data['Prime Mover']))
#PMs.pop(0)
#PM_data = graphing_data[graphing_data['Technology'] != 'NaN']
#fig, ax = plt.subplots()
#   
#i = 0
#for tech in PMs:
#    specific_tech_data = PM_data[PM_data['Prime Mover'] == tech]
#    
#    fraction = specific_tech_data['Fraction'] *100
#    if np.average(fraction) > 0.75:
#        plt.plot(years, fraction, label=tech, linewidth = 3, color= colors[i])
...
#ax.set_ylabel('Percent')
#
#plt.savefig('PM_graph1.png', dpi=400)
#plt.show()     
#    